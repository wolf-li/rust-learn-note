# enumerations (enum) 枚举

enumerations 也称 enums，枚举允许列举可能的成员来定义一个类型。
用户自定义类型，用于表示具有一组离散可能性的变量。  
...
// 联合函数
impl Building{
    fn print_building(&self){
        match self{
            Building::Name(c) => println!("building number {}", c),
            Building::Number(s) => println!("building number {}", s),
            Building::Unknow => println!("unknow"),
        }
    }
}

# fn main(){
let house = Building::Name(String::from("five street"));
let house = Building::Number(13);
let house = Building::Unknow;
